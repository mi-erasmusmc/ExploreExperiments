...
server <- function(input, output, session) {
  ...
  output$dynamic_covariates = renderUI({
    ...
    two <- selectInput("covariates", label = "Covariates", choices = c("Phenotypes", "Full", "Reduced"), selected = "Phenotypes")
  ...
  from_csv_explore_options <- reactive({
    input_data <- read.csv(file.path(outputFolder, input$resultFolder, "explore_options_RT.csv"))
  ...
  output$exploreOptionsSummary <- renderDataTable({
    table <- from_csv_explore_options()

    # Parallel = TRUE
    tab1 <- table %>% filter(Parallel==TRUE) %>% group_by(ParallelMethod) %>% summarise(avg_time = mean(Time)) %>% mutate(compare=paste0("Parallel = TRUE, loop: ", ParallelMethod), perc=(min(avg_time)-max(avg_time))*100.0/(max(avg_time)))
    tab1$perc[tab1$avg_time==max(tab1$avg_time)] <- NA

    # Parallel
    tab2 <- table %>% filter(ParallelMethod=="TWO") %>% group_by(Parallel) %>% summarise(avg_time = mean(Time)) %>% mutate(compare=paste0("Parallel: ", Parallel, " (loop: TWO)"), perc=(min(avg_time)-max(avg_time))*100.0/(max(avg_time))) # TODO: for FALSE, make default TWO?
    tab2$perc[tab2$avg_time==max(tab2$avg_time)] <- NA

    # Sorted time
    tab3 <- table %>% filter(Parallel==TRUE, ParallelMethod=="TWO") %>% group_by(Sorted) %>% summarise(avg_time = mean(Time)) %>%  mutate(compare=paste0("Parallel = TRUE, sorted: ", Sorted), perc=(min(avg_time)-max(avg_time))*100.0/(max(avg_time)))
    tab3$perc[tab3$avg_time==max(tab3$avg_time)] <- NA

    summary_table <- rbind(tab1[c("compare", "avg_time", "perc")], tab2[c("compare", "avg_time", "perc")], tab3[c("compare", "avg_time", "perc")])
  ...
  output$exploreOptionsSummaryTitle <- renderText({
    paste0("Explore results for output ", names(which(resultFolders == input$resultFolder)))
  })
  ...
  output$exploreOutputSorted <- renderPlotly({
    plot_data <- from_csv_explore_options()

    figure <- ggplot(plot_data, aes(Sorted, Time, shape = Parallel, color = Data, text = paste("Data: ", Data,
                                                                                                 "<br>Time: ", Time))) +
      geom_point() +
      theme_bw() +
      scale_y_log10(labels = scales::comma) +
  ...
  output$exploreOutputParallel <- renderPlotly({
    plot_data <- from_csv_explore_options()

    figure <- ggplot(plot_data, aes(Parallel, Time, shape = ParallelMethod, color = Data, text = paste("Data: ", Data,
                                                                                               "<br>Time: ", Time))) +
      geom_point() +
      theme_bw() +
      scale_y_log10(labels = scales::comma) +
  ...
  output$exploreOutputParallelMethod <- renderPlotly({
    plot_data <- from_csv_explore_options()

    figure <- ggplot(plot_data, aes(ParallelMethod, Time, shape = Sorted, color = Data, text = paste("Data: ", Data,
                                                                                                       "<br>Time: ", Time))) +
      geom_point() +
      theme_bw() +
      scale_y_log10(labels = scales::comma) +
  ...
  output$exploreOutputBranchBound <- renderPlotly({
    plot_data <- from_csv_explore_options()

    figure <- ggplot(plot_data, aes(BranchBound, Time, shape = Parallel, color = Data, text = paste("Data: ", Data,
                                                                                                 "<br>Time: ", Time))) +
      geom_point() +
      theme_bw() +
      scale_y_log10(labels = scales::comma) +
  ...
  output$aucCurves <- renderPlotly({
    ...
    exclude <- c()
    # exclude <- c("DecisionTree_2", "DecisionTree_3", "IHT_5", "IHT_10")

    output <- output[!(output$group %in% exclude),]

    output <- output[order(output$values_TPR, decreasing = FALSE),] # Result can be non-monotonically increasing for test set
  ...
  output_methods_metrics_pt <- reactive({
    ...
    output <- output[order(output$values_threshold, decreasing = FALSE),] # Result can be non-monotonically increasing for test set
  ...
  output$nbPlot <- renderPlotly({
    ...
    figure <- ggplot2::ggplot(
      ...
      xlim(0,0.4) + xlab("Risk threshold (p)") + ylab("Net benefit") + scale_linetype_manual(values = group.lines) # + scale_color_manual(values=group.colors)
