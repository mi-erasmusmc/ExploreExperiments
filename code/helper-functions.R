...
evaluateModel <- function(predictions, real, model=NULL, class=T) {
  ...
  if (class) { # Return for classes
    ...
    brier <- brier_score(predictions,real)$brier
    ...
    for (c in 1:length(curve_thresholds)) {
      ...
      curve_brier[c] <- eval$Perf_Calibration
    }
    ...
    results <- list(Perf_AUC= pracma::trapz(curve_FPR[length(curve_FPR):1],curve_TPR[length(curve_TPR):1]), # roc$auc,
                    ...
                    Curve_Calibration=paste(curve_brier, collapse = "_"),
                    Curve_Models=ifelse(is.null(model), NA, model),
                    N_outcomes=sum(real == 1),
                    N_controls=sum(real == 0),
                    N_total=length(real))
...
evaluateExplore <- function(modelsCurve, plpModel, data, train) {
  ...
  if (!train) {
    ...
    # Apply needed data transformations
    if(!is.null(plpModel$preprocessing$tidyCovariates)){
      # do preprocessing
      data$covariateData <- do.call(
        PatientLevelPrediction:::applyTidyCovariateData,
        list(
  ...
  exploreData <- merge(data$labels[c("rowId", "outcomeCount")], denseData, by = 'rowId', all.x = TRUE)
  ...
  curve_brier <- c(1,1) # TODO: which edge cases to include?

  # (automatic tidyCovariates etc.)

  for (c in length(modelsCurve):1) {
    model <- modelsCurve[c]

    # Predict using train and test
    prediction <- tryCatch(as.numeric(Explore::predictExplore(model = model, test_data = exploreData)))

    # Cannot use PLP predict function directly as want to compute for all models on curve
    # predict <- PatientLevelPrediction::predictPlp(plpModel = plpModel, plpData = data, population = data$labels)
    # prediction <- predict$value

    # Compute metrics
    eval_curve <- evaluateModel(prediction, data$labels$outcomeCount)
    ...
    curve_brier[c+2] <- eval_curve$Perf_Calibration
  }
...
parse_time <- function(time_string) {
  match <- stringr::str_match(time_string, "^([0-9.]+)\\s*(secs?|mins?|hours?)$")
  value <- as.numeric(match[2])
  unit <- match[3]

  duration_mins <- switch(tolower(unit),
                     secs=value/60,
                     mins=value,
                     hours=value*60)
}
...
# Borrowed from PatientLevelPrediction package.
brier_score <- function(pred, class){
  brier <- sum((class-pred)^2)/length(pred)
